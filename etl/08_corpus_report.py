...
def get_prototype_articles(config: util.Config) -> dict:
    query = f"""
    select
        id,
        "name"
    ...
    where
        articles_corpus_article.read_status = 'Prototype'
    """
...
def get_distances(config: util.Config) -> list:
    ...
    select
        ...
        case
            when articles_corpus_article.read_status = 'Prototype' then 'Yes'
            else ''
        end read_status,
        articles_corpus_article.percent_about,
        articles_corpus_article."name" "Article",
        1 - exp(sum(ln(1 + 1e-320 - articles_corpus_article_corpus_article.distance))) "Relevance"
    ...
    group by
        articles_corpus_article.id,
        articles_corpus_article_corpus_article.corpus_article_id,
...
def get_articles_by_prototype(prototype_article_id: int, config: util.Config) -> tuple:
    ...
    select
        ...
        alpha,
        beta
    from articles_corpus_article articles_prototype_article
    inner join articles_corpus_article_corpus_article on
        articles_corpus_article_corpus_article.prototype_article_id = articles_prototype_article.id and
...
def get_calculation_sheet(workbook: Workbook, config: util.Config):
    query = f"""
    select
        id,
        "name"
    ...
    distances = {
        ...
        for article_id, rank, journal, year, read_status, percent_about, name, distance in get_distances(config)
    }
...
def get_individual_sheets(workbook: Workbook, config: util.Config):
    query = f"""
    select
        id,
        "name"
...
if __name__ == '__main__':
    ...
    workbook.save(f'{topic_name} Iteration 12.xlsx')
