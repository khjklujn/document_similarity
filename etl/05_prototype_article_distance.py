...
class State:
    ...
    prototype_articles = repository.prototype.PrototypeArticle(util.Config())
    prototype_article_corpus_articles = repository.prototype.PrototypeArticleCorpusArticle(util.Config())
    prototype_article_stems = repository.prototype.PrototypeArticleStem(util.Config())
...
def calculate_distance(prototype_article_id: int, corpus_article: model.odoo.corpus.CorpusArticle, state: State) -> float:
    prototype_stems = {
        ...
        for stem in state.prototype_article_stems.by_prototype_article_id(prototype_article_id)
    ...
    if len(corpus_stems) > 10:
        ...
        prototype_vector = np.array(
            [
                prototype_stems.get(stem_id, 1e-100 + 0.0j)
                for stem_id in stems
            ]
        )
        corpus_vector = np.array(
            [
                corpus_stems.get(stem_id, 1e-100 + 0.0j)
                for stem_id in stems
            ]
        )
...
def process_prototype_article(prototype_article: model.odoo.prototype.PrototypeArticle, corpus_id: int, state: State):
    ...
    for jenson_shannon, corpus_article_id, journal_id, year_id in similarities:
        ...
        if count % 200 == 0:
            ...
            state.prototype_article_corpus_articles.upsert_many()
        count += 1
...
def main():
    ...
    topic_id = 5
    corpus_id = 1
    for article in state.prototype_articles.by_topic_id(topic_id):
